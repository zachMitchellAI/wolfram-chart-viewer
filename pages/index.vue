...
<template>
  <v-app theme="mdi-weather-night">
    <v-container fluid class="fg-surface-variant h-100">
      <v-app-bar class="bar"
        :title="'Wolfram Chart Viewer' + (chartData.activeDataset?.shortenedQuery ? ' - ' + chartData.activeDataset?.shortenedQuery : '')"></v-app-bar>
      <v-row gap="16" size="5">
        <v-col cols="4">
          <!-- ChartJS goes here! -->
        ...
        <v-col cols="1">
          <v-sheet elevation="2">
            <v-tabs color="primary" v-model="tab">
              <v-tab v-for="coll in chartData.collections" :value="coll.name.toLowerCase().replaceAll(' ', '-')">
                {{ coll.name }}
              </v-tab>
            </v-tabs>
            ...
            <v-tabs-window v-model="tab">
              ...
              <v-tabs-window-item v-for="coll in chartData.collections" :key="coll.name"
                :value="coll.name.toLowerCase().replaceAll(' ', '-')">
                <v-sheet class="pa-5" v-if="coll.queriable">
                  <v-textarea label="I'm an AI with an MCP server! Ask me in plain english!" variant="solo"
                ...
                <v-data-iterator :items="coll.entries" item-value="shortenedQuery" :items-per-page="-1">
                  <template v-slot:default="{ items, isExpanded, toggleExpand }">
                    <v-row>
                      <v-col v-for="item in items" :key="item.raw.shortenedQuery" cols="12">
                        <v-skeleton-loader class="border" :type="['image', 'heading', 'text']"
                          :loading="item.raw.loading" v-if="item.raw.loading"></v-skeleton-loader>

                        <v-card :variant="chartData.activeDataset === item.raw ? 'tonal' : 'elevated'"
                          class="cursor-pointer" @click="chartData.setActiveDataset(item.raw)" v-else>
                          <v-card-title class="d-flex align-center">
                            <v-icon color="primary" icon="mdi-chart-box" size="18" start></v-icon>
                          ...
                          <div class="px-4">
                            <v-switch :label="`${isExpanded(item) ? 'Hide' : 'Show'} details`"
                              :model-value="isExpanded(item)" density="compact" inset
                              @click.stop="() => toggleExpand(item)"></v-switch>
                          </div>
                          ...
                          <v-expand-transition>
                            <div v-if="isExpanded(item)">
                              <v-list :lines="false" density="compact">
                                ...
                                <v-list-item
                                  :title="`Labels: ${item.raw.dataset.data.labels?.join(', ')}`"></v-list-item>
                                <v-list-item v-for="(dataset, index) in item.raw.dataset.data.datasets" :key="index"
                                  :title="`Dataset ${index + 1}: ${dataset.label || 'Unlabeled'}`"></v-list-item>
                              </v-list>
                            </div>
                          </v-expand-transition>
                        </v-card>
...
async function queryWolfram(evt) {
  ...
  const finsishedDataset = await chartData.queryNewDatset(evt.target.value);
